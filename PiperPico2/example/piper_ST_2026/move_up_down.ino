...
)
{
    ...
    */
    ...
    if (pulsesPerCmUsed <= 0.0f)
    {
        ...
        Serial.println(
            "MOVE ERROR: invalid pulsesPerCm"
        );
    ...
    /*
       ความเร็วขั้นต่ำ
    */
    const int MIN_SPEED = 18;

    /*
       ความเร็วปลายทาง
    */
    const int FINAL_SPEED = 18;

    /*
       ระยะหยุด

       ไม่ใช้ 4 cm
       และไม่ใช้ 1.5 cm แบบเดิม

       ให้เริ่มชะลอโดย profile
       แล้วหยุดเมื่อเหลือน้อยมาก
    */
    const float STOP_DISTANCE = 0.8f;
    ...
    /*
       ...
       ไม่ใช้ SLOW_DISTANCE แบบ fixed

       กำหนดระยะ braking จากความเร็ว
    */

    float brakeDistance;

    if (requestedSpeed >= 90.0f)
    {
        brakeDistance = 22.0f;
    }
    else if (requestedSpeed >= 75.0f)
    {
        brakeDistance = 18.0f;
    }
    else if (requestedSpeed >= 60.0f)
    {
        brakeDistance = 15.0f;
    }
    else
    ...
    /*
       สำหรับระยะสั้น
       ห้าม brakeDistance ใหญ่เกินครึ่งหนึ่ง
    */
    ...
    int lastLeftMotor = leftBase;

    int lastRightMotor = rightBase;

    //====================================================
    // LINE STATE
    //====================================================

    const int LINE_HYSTERESIS = 5;

    bool lineLeftState = false;

    bool lineRightState = false;
    ...
    Serial.print(" cm | BrakeDist = ");
    Serial.print(brakeDistance, 1);

    Serial.print(" cm | AccelDist = ");
    Serial.print(accelDistance, 1);

    Serial.print(" cm | Speed = ");
    Serial.println(requestedSpeed, 1);
    ...
    while (true)
    {
        ...
        /*
           ถ้ายังไม่ถึง braking zone
           วิ่งความเร็วเต็ม
        */

        else if (
            remain >= brakeDistance
        )
        {
            baseLeft =
                leftBase;
        ...
        )
        {
            ...
            const int thresholdLeft = robot.adcMin(8);          

            const int thresholdRight = robot.adcMin(1);
        ...
        )
        {
            ...
            const int thresholdLeft = robot.adcMin(9);

            const int thresholdRight =  robot.adcMin(0);
...
void move_to_can(int sl, int sr, float kp, float dist, float arm_target)
{
    ...
    long startPulses = encoder1.Poss_R();
...
void move_bridge(
    ...
    int sr,             // ความเร็วมอเตอร์ขวา
    float kp,           // Kp ควบคุมทิศทาง
    float dist,         // ระยะทางเป้าหมาย cm
    int offset,         // Brake pulse ms, 0 = ไม่เบรก
    float arm_target,   // ตำแหน่งแขนเป้าหมาย
    float deg            // มุมเป้าหมายจริง
)
{
    ...
    //====================================================
    // ปรับระยะตามสถานะเดิม
    //====================================================

    /*
       offset == 0
       ลดระยะลง 5 cm ตามพฤติกรรมเดิม
    */
    ...
    */
    ...
    //====================================================
    // ENCODER
    // ใช้ Encoder ซ้าย + ขวา
    // ไม่เปลี่ยน calibration
    //====================================================

    encoder1.resetEncoders();

    const long startPulsesL =
        encoder1.Poss_L();

    const long startPulsesR =
        encoder1.Poss_R();
    ...
    //====================================================
    // BRAKE / DECELERATION DISTANCE
    //====================================================

    float brakeDistance;

    if (averageBaseSpeed >= 90.0f)
    {
        brakeDistance = 22.0f;
    }
    else if (averageBaseSpeed >= 75.0f)
    {
        brakeDistance = 18.0f;
    }
    else if (averageBaseSpeed >= 60.0f)
    {
        brakeDistance = 15.0f;
    }
    else
    ...
    /*
       ป้องกัน brakeDistance ใหญ่เกินไป
       สำหรับระยะทางสั้น
    */
    ...
    Serial.print(" | AccDistance = ");
    Serial.print(ACC_DISTANCE, 1);
    ...
    while (true)
    {
        ...
        const long currentPulsesL =
            encoder1.Poss_L();

        const long currentPulsesR =
            encoder1.Poss_R();
        ...
        const float averagePulses =
            (
                (float)pulseDifferenceL +
                (float)pulseDifferenceR
            ) * 0.5f;
        ...
        /*
           ...
           ความเร็วต่ำ
           correction ยังเหลืออย่างน้อย 65%
        */
        ...
        /*
           เหลือ 5 cm สุดท้าย
           ลด correction เพื่อป้องกันสะบัด
        */
        ...
        if (lineActive)
        {
            ...
            /*
               ช่วงชะลอ
               ให้ Gyro เป็นตัวหลัก
            */

            if (remain < brakeDistance)
            {
                lineFactor = 0.45f;
            }
    ...
    const long finalPulsesL =
        encoder1.Poss_L();

    const long finalPulsesR =
        encoder1.Poss_R();

    const long finalDifferenceL =
        labs(
            finalPulsesL -
            startPulsesL
        );

    const long finalDifferenceR =
        labs(
            finalPulsesR -
            startPulsesR
        );

    const float finalAveragePulses =
        (
            (float)finalDifferenceL +
            (float)finalDifferenceR
        ) * 0.5f;
    ...
    Serial.print(finalDifferenceL);

    Serial.print(" | Encoder R = ");
    Serial.print(finalDifferenceR);

    Serial.print(" | Pulse Avg = ");
    Serial.print(finalAveragePulses, 1);
